...
- **In scope (the cryptographic core):**
  - `frontend/src/crypto/e2ee.ts` — AES-256-GCM, scrypt KDF, DEK/KEK wrapping,
    recovery code, escrow lifecycle.
  - `frontend/app/_layout.tsx` — CSPRNG polyfill ordering.
...
4. **KDF:** scrypt params are adequate for a mobile passphrase KEK; per-use random
   salts; no key/salt reuse across purposes.
...
- Are the scrypt parameters appropriate given the threat model and target devices?
...
- **On-device:** `/crypto-check` route runs the same checks + scrypt benchmark +
...
- ✅ 256-bit DEK from CSPRNG; scrypt with random salt.
- ✅ No hardcoded keys; no plaintext/keys in logs or analytics paths.
- ✅ Reset preserves DEK; old password invalidated.
- ⚠️ Open items for an auditor: scrypt cost tuning per device; consider per-purpose
