...
Railway watches `nueco-backend`, a separate repo nested inside this monorepo's `backend/` folder — not the repo this session works in. A commit here does nothing in production until it's mirrored across by hand, which has already caused at least one silent-404 incident.
